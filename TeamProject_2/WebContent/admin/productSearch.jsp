...
<div class="container" style=" text-align: center; margin-bottom:60px;">
    <div class="section-title product__discount__title" style=" text-align: center;">
		...
		<form action="/TeamProject/admin/adminController?type=searchProduct" method="post">
		        ...
		        <tbody>
					...
					<c:forEach var="bvo" items="${productList }">
						<tr>
							...
							<td>${bvo.category }</td>
							<td>${bvo.id }</td>
							<td><a href="/TeamProject/admin/adminController?type=productIdx&boardIdx=${bvo.boardIdx}">${bvo.title }</a></td>
							<td>${bvo.content }</td>
							<td>${bvo.price }</td>
							<td>${bvo.status }</td>
							<td>${bvo.likes }</td>
